                            ...
                            <div class="col-sm-12">
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        <div class="card-header-right"><a href="{{ route('admin.category.store') }}" type="button" class="btn btn-primary btn-sm">Criar nova categoria</a>
                                                ...
                                                <div class="table-responsive">
                                                    <table id="responsive-table" class="display table dt-responsive nowrap" style="width:100%">
                                                        ...
                                                        <tbody>
                                                            ...
                                                            <tr>
                                                                <td>{{ $i }}</td>
                                                                <td><img src="{{ url("storage/".$category->image) }}" class="rounded float-start" width="70em" height="40em"></td>
                                                                <td>{{ $category->name }}</td>
                                                                <td>{{ $category->description }}</td>
                                                                <td>{{ date('d/m/Y', strtotime($category->created_at)) }}</td>
                                                                <td>
                                                                    <form method="post" action="{{ route('admin.category.destroy', $category->id) }}">
                                                                    <a href="{{ route('admin.category.edit', $category->id) }}" type="button" class="btn btn-primary btn-sm">Editar</a>
                                                                    @csrf
